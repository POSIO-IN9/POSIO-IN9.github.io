...
const greeting = {
  ...
  myname: "최인규입니다.",
  subTitle: "A passionate Full Stack SW Developer🚀 ",

  subTitle2:
    "Web 백엔드 개발(java), LLM-RAG 기반 Ai 서비스, objectdetection 커스텀데이터 셋 학습 모델 구축, GCP를 활용한 클라우드 서비스 구축. 다양한 활동으로 개발이력을 쌓아 왔습니다. 백엔드 web개발자, AI 개발자가 되기 위해 열정과 배움으로 나아갑니다.",
  subTitle3:
    "'함께'라는 가치를 알고 서로가 유기체적으로 얽히며 나아가는 세상을 꿈꿉니다. 여러 학문을 배우며 넓혀진 시야로 목표를 관철합니다. 서비스 개발에 있어 새로운 인사이트를 가져올 수 있는 인재, 맡은 일에 사명을 다하는 인재가 되겠습니다.",
  subTitle4:
    "인과연이 스칠 때마다 배움의 발자취를 남깁니다. 깊게 딛어 움푹한 자국들이 모여 지식의 깊이를 가늠합니다. 느릴지언정 전진을 멈춘 적은 없습니다. 이제 달리는 법을 깨치려 합니다.",
  resumeLink:
  ...
  skills: [
    {
      ...
      content:
        "React,tailwind를 사용하여 js에서의 신속한 동적 웹 스타일링 구성.구성별 컨포넌트화 관리. fetch와 axios등을 통해 백엔드 연동 함수 구현. api로 전달받은 데이터를 hook,state,recoil등을 이용한 js 동적 로직 구현."
    },
    {
      id: 2,
    ...
    },
    {
      ...
      content:
        "pandas라이브러리 활용한 데이터셋 전처리 및 radomforest를 통한 분류학습. LLM-RAG 기반 ai챗봇모델 파이프라인 설계 및 서비스 구축. 다국적언어모델(bert)를 이용한 NLP 감성분석 파이프라인 설계. 시계열 데이터를 활용한 LSTM 시험 모델 구축, yolo8을 활용한 커스텀 데이터 obectdetection 시험 모델 구축. 모델과 백엔드와의 파이프라인 설계. skyline-query, topsis(다목적의사결정) 알고리즘 작성을 통해 위험도 분석 서비스."
    },
    {
      id: 4,
      skill: "cloud-service{Gcp,docker}",
      content:
        "Gcp 구글 클라우드를 통해 sql과 google-storage를 서비스에 연결. docker를 사용하여 node.js기반 npm build 이미지화, gradle kts기반 bootjar 이미지화로 서비스 배포. docker-compose를 통한 관리. sh를 통해 ai모델의 빌드를 자동화 및 cerbot을 이용한 domain 인증서 자동화. nginx를 통해 도메인 프록시 연결. vm머신 ssh를 통해 linux환경에서 작업."
    }
  ],
...
const educationInfo = {
  ...
  schools: [
    ...
    },
    {
      ...
      descBullets: [
        "부산대학교 홍봉희 지도교수님의 가르침 아래서 짦은 시간이지만 역력한 개발 실력을 쌓을 수 있었습니다.",
  ...
  projects: [
    {
      ...
      projectDesc:
        "LLM-LAG기반 AI를 활용한 국민 생애 직업주기에 맞춘 교육 추천,상담 및 교육기관 관리",
      projectsubtitle:
        "맡은 부분: 백엔드 및 AI모델 파이프라인, AI모델 구축 보조, 서비스 배포",
      footerLink: [
        {
          name: "시연영상",
